...
import { APIKeyGeneration, TokenGeneration } from "@/src/public/models/data_classes/auth";
...
export async function DELETE(request: Request, response: Response) {
  try {
    ...
    const key = await apiClient.deleteAPIKey(credentials.data.user_id, credentials.data.token);
    if (!key) {
      return generateServerResponses("API Key Deletion Failed", 500);
    }

    return generateServerResponses(key, 200);
  } catch (error) {
    console.log(`API Key Error: ${error}`);
